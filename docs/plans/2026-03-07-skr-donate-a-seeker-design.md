...
Add a 4th campaign option — "Donate a Seeker" — that accepts $SKR token donations. When the campaign is selected, the Give flow switches from USDC to SKR. The transaction uses the same `transferChecked` + Memo pattern, same MWA signing flow, same server-side validation. SKR donors earn 2x points in Seasons 1 & 2.
...
- "SKR donors earn 2x points in Seasons 1 & 2" badge
- Minimum: 100 SKR
...
Changes:
1. Add `SKR_MINT` constant and `VALID_SKR_MINTS` set
2. Add `MATCHING_POOL_SKR_ATA` constant (pre-derived)
3. Create `VALID_MINTS` map: `mint → { ata, decimals, symbol, maxAmount }`
4. Validation function becomes mint-aware:
   - Look up mint in `VALID_MINTS` → get expected ATA + decimals
...
6. `amount_usdc` column used for USDC; new `amount_skr` column for SKR
...
**Not implemented in code for hackathon.** The campaign card displays "2x points in Seasons 1 & 2" as messaging. The leaderboard (Rank tab) is already a placeholder. Weighting logic ships post-hackathon when the leaderboard goes live.
...
- Retry queue logic (AsyncStorage)
...
> "Glimpse is the first app where you can donate your SKR to put a Seeker in someone's hands. Every SKR contributed goes toward funding a device for someone who needs it. When the campaign hits $500, we buy the Seeker and ship it — then you see the proof and start a conversation with the person who received it. SKR donors earn 2x points in our first two seasons."
...
| `screens/GiveScreen.tsx` | Edit | Conditional labels, prefix, validation, USD conversion display, 2x badge |
| `supabase/functions/record-donation/index.ts` | Edit | Multi-token mint validation, SKR ATA, campaign rule, amount_skr column |
| `supabase/functions/helius-webhook/index.ts` | Edit | SKR mint detection, memo tok=skr, amount_skr |
| `supabase/migrations/018_skr_donations.sql` | Create | Add `amount_skr`, `token_type` columns |
